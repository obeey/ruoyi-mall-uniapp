...
const contactTagStyle = {
  img: "max-width: 640rpx !important;",
...
const thirdLogin = async (provider, event = null) => {
  ...
  const loginRes = await sheep.$platform
    .useProvider(provider)
    .login(event?.detail || null);
  if (loginRes) {
    closeAuthModal();
...
watch(
  ...
  async () => {
    ...
    if (authType.value === "contact") {
      const res = await sheep.$api.data.getSysConfig({
        configKey: "mall.contact",
      });
      contact.value = res.data || "";
    }
    preAuthType.value = authType.value;
  }
);
</script>
